...
function syncCicloToState(ciclo, reputacao, fundo, setCicloId, setSolicitacao, setCicloEstado, setAvaliacoes, setEvidencia, setOficinaConfirmacao, setGestaoJustificativa, setReputacao, setFundo) {
  if (!ciclo) return
  setCicloId(ciclo.id)
  localStorage.setItem('abias_ciclo_id', ciclo.id)
  setSolicitacao({ valor: ciclo.valor, finalidade: ciclo.finalidade, prazo: ciclo.prazo, oficina: ciclo.oficina, urgencia: ciclo.urgencia, descricao: ciclo.descricao })
...
function App() {
  // --- IDs persistidos no localStorage para restaurar sessão ---
  ...
  // --- Estado da aplicação (fonte de verdade: backend) ---
  ...
  const [parecerAi, setParecerAi] = useState({})
  const [parecerAiLoading, setParecerAiLoading] = useState({})
  ...
  useEffect(() => {
    ...
    async function carregarEstado() {
      try {
        ...
        if (ciclo) {
          syncCicloToState(ciclo, undefined, undefined, setCicloId, setSolicitacao, setCicloEstado, setAvaliacoes, setEvidencia, setOficinaConfirmacao, setGestaoJustificativa, setReputacao, setFundo)
        }

        const { fundo: f } = await apiFetch('/abias/fundo')
        setFundo(f.saldo)

        apiFetch(`/abias/membros/${membroId}/credito`)
          .then(d => setCreditoData(d.credito))
          .catch(() => {})
      } catch {
  ...
  // --- Gestão: carrega painel iFood ao entrar no modo gestão ---
  useEffect(() => {
    if (profileMode !== 'gestao') return
    setGestaoLoading(true)
    apiFetch('/abias/gestao/painel')
      .then(data => setGestaoData(data.membros))
      .catch(() => setGestaoData(null))
      .finally(() => setGestaoLoading(false))
  }, [profileMode])
  ...
  const handleCadastro = async (e) => {
    ...
    try {
      ...
      alert('Erro ao registrar membro: ' + err.message)
    }
  }
  ...
  const handleSolicitacaoSubmit = async (e) => {
    ...
    try {
      ...
      syncCicloToState(data.ciclo, data.reputacao, data.fundo, setCicloId, setSolicitacao, setCicloEstado, setAvaliacoes, setEvidencia, setOficinaConfirmacao, setGestaoJustificativa, setReputacao, setFundo)
    } catch (err) {
  ...
  const handleConfirmarPlano = async () => {
    try {
      const data = await apiFetch(`/abias/ciclos/${cicloId}/estado`, {
        method: 'PATCH',
        body: { estado: 'community_validation' }
      })
      syncCicloToState(data.ciclo, data.reputacao, data.fundo, setCicloId, setSolicitacao, setCicloEstado, setAvaliacoes, setEvidencia, setOficinaConfirmacao, setGestaoJustificativa, setReputacao, setFundo)
    } catch (err) {
  ...
  const handleMarcosAval = async () => {
    try {
      const data = await apiFetch(`/abias/ciclos/${cicloId}/avais`, {
        method: 'POST',
        body: { avaliadoPor: 'marcos', comentario: 'João é parceiro de confiança, conheço a rota dele na Zona Leste.' }
      })
      syncCicloToState(data.ciclo, data.reputacao, data.fundo, setCicloId, setSolicitacao, setCicloEstado, setAvaliacoes, setEvidencia, setOficinaConfirmacao, setGestaoJustificativa, setReputacao, setFundo)
    } catch (err) {
      alert('Erro: ' + err.message)
  ...
  const handleAlineAval = async () => {
    try {
      const data = await apiFetch(`/abias/ciclos/${cicloId}/avais`, {
        method: 'POST',
        body: { avaliadoPor: 'aline', comentario: 'Grande profissional de entrega. Corre garantido no asfalto.' }
      })
      syncCicloToState(data.ciclo, data.reputacao, data.fundo, setCicloId, setSolicitacao, setCicloEstado, setAvaliacoes, setEvidencia, setOficinaConfirmacao, setGestaoJustificativa, setReputacao, setFundo)
    } catch (err) {
      alert('Erro: ' + err.message)
  ...
  const handleOficinaConfirmarOrçamento = async () => {
    try {
      const data = await apiFetch(`/abias/ciclos/${cicloId}/confirmacoes`, {
        method: 'POST',
        body: { tipo: 'orcamento' }
      })
      syncCicloToState(data.ciclo, data.reputacao, data.fundo, setCicloId, setSolicitacao, setCicloEstado, setAvaliacoes, setEvidencia, setOficinaConfirmacao, setGestaoJustificativa, setReputacao, setFundo)
  ...
  const handleGestaoPedirRevisao = async () => {
    ...
    try {
      const data = await apiFetch(`/abias/ciclos/${cicloId}/estado`, {
        method: 'PATCH',
        body: { estado: 'needs_revision', justificativa: adminJustifyInput }
      })
      syncCicloToState(data.ciclo, data.reputacao, data.fundo, setCicloId, setSolicitacao, setCicloEstado, setAvaliacoes, setEvidencia, setOficinaConfirmacao, setGestaoJustificativa, setReputacao, setFundo)
  ...
  const handleGestaoRecusar = async () => {
    ...
    try {
      const data = await apiFetch(`/abias/ciclos/${cicloId}/estado`, {
        method: 'PATCH',
        body: { estado: 'rejected', justificativa: adminJustifyInput }
      })
      syncCicloToState(data.ciclo, data.reputacao, data.fundo, setCicloId, setSolicitacao, setCicloEstado, setAvaliacoes, setEvidencia, setOficinaConfirmacao, setGestaoJustificativa, setReputacao, setFundo)
      setAdminJustifyInput('')
    } catch (err) {
  ...
  const handleEnviarEvidencia = async () => {
    ...
    try {
      const data = await apiFetch(`/abias/ciclos/${cicloId}/evidencias`, {
        method: 'POST',
        body: { arquivo: uploadFileSelected, observacao: uploadFileObs, tipo: uploadFileType }
      })
      syncCicloToState(data.ciclo, data.reputacao, data.fundo, setCicloId, setSolicitacao, setCicloEstado, setAvaliacoes, setEvidencia, setOficinaConfirmacao, setGestaoJustificativa, setReputacao, setFundo)
    } catch (err) {
  ...
  const handleSolicitarParecerAi = async (membroId) => {
    setParecerAiLoading(prev => ({ ...prev, [membroId]: true }))
    try {
      const data = await apiFetch(`/abias/gestao/ai-analise/${membroId}`)
      setParecerAi(prev => ({ ...prev, [membroId]: data }))
    } catch (err) {
      alert('Erro ao solicitar parecer: ' + err.message)
    } finally {
      setParecerAiLoading(prev => ({ ...prev, [membroId]: false }))
    }
  }
                    ...
                    <>
                      ...
                      {activeTab === 'inicio' && (
                        <div className="screen active" id="screen-home">
                          ...
                          {/* Cartão de crédito disponível — visão bancária */}
                          {creditoData && (
                            <div style={{ margin: '16px 20px 0', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                              <span style={{ fontSize: '0.62rem', fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Seu crédito disponível</span>
                              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                                {/* Cartão */}
                                <div style={{
                                  padding: '14px 12px',
                                  background: creditoData.limites?.cartao?.disponivel ? 'rgba(22,61,47,0.25)' : 'rgba(255,255,255,0.02)',
                                  border: `1px solid ${creditoData.limites?.cartao?.disponivel ? 'rgba(34,197,94,0.3)' : 'var(--border-color)'}`,
                                  borderRadius: '6px'
                                }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '6px' }}>
                                    <i className="fa-solid fa-credit-card" style={{ fontSize: '0.7rem', color: creditoData.limites?.cartao?.disponivel ? 'var(--color-green)' : 'var(--text-muted)' }}></i>
                                    <span style={{ fontSize: '0.6rem', color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase' }}>Cartão</span>
                                  </div>
                                  <div style={{ fontSize: '1.15rem', fontWeight: 900, color: creditoData.limites?.cartao?.disponivel ? 'var(--color-green)' : 'var(--text-muted)', fontFamily: 'monospace', lineHeight: 1 }}>
                                    {creditoData.limites?.cartao?.disponivel ? `R$ ${creditoData.limites.cartao.valor.toLocaleString('pt-BR')}` : '—'}
                                  </div>
                                  <div style={{ fontSize: '0.58rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                                    {creditoData.limites?.cartao?.disponivel ? 'Limite rotativo' : 'Indisponível'}
                                  </div>
                                </div>
                                {/* Empréstimo */}
                                <div style={{
                                  padding: '14px 12px',
                                  background: creditoData.limites?.emprestimo?.disponivel ? 'rgba(201,154,61,0.12)' : 'rgba(255,255,255,0.02)',
                                  border: `1px solid ${creditoData.limites?.emprestimo?.disponivel ? 'rgba(201,154,61,0.35)' : 'var(--border-color)'}`,
                                  borderRadius: '6px'
                                }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '6px' }}>
                                    <i className="fa-solid fa-hand-holding-dollar" style={{ fontSize: '0.7rem', color: creditoData.limites?.emprestimo?.disponivel ? 'var(--color-gold)' : 'var(--text-muted)' }}></i>
                                    <span style={{ fontSize: '0.6rem', color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase' }}>Empréstimo</span>
                                  </div>
                                  <div style={{ fontSize: '1.15rem', fontWeight: 900, color: creditoData.limites?.emprestimo?.disponivel ? 'var(--color-gold)' : 'var(--text-muted)', fontFamily: 'monospace', lineHeight: 1 }}>
                                    {creditoData.limites?.emprestimo?.disponivel ? `R$ ${creditoData.limites.emprestimo.valor.toLocaleString('pt-BR')}` : '—'}
                                  </div>
                                  <div style={{ fontSize: '0.58rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                                    {creditoData.limites?.emprestimo?.disponivel ? 'Crédito produtivo' : (creditoData.limites?.emprestimo?.motivo || 'Indisponível')}
                                  </div>
                                </div>
                              </div>
                              {/* Score iFood */}
                              {creditoData.scoreIfood && (
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border-color)', borderRadius: '4px' }}>
                                  <span style={{ fontSize: '0.68rem', color: 'var(--text-secondary)' }}>
                                    <i className="fa-solid fa-chart-line" style={{ marginRight: '5px', color: 'var(--color-gold)' }}></i>
                                    Score operacional iFood
                                  </span>
                                  <div style={{ textAlign: 'right' }}>
                                    <span style={{ fontSize: '0.8rem', fontWeight: 900, fontFamily: 'monospace', color: '#ffffff' }}>{creditoData.scoreIfood}</span>
                                    <span style={{ fontSize: '0.6rem', color: 'var(--text-muted)', marginLeft: '5px' }}>{creditoData.classificacaoIfood}</span>
                                  </div>
                                </div>
                              )}
                            </div>
                        ...
                        <div className="screen active" id="screen-solicitar">
                          ...
                          {cicloEstado === 'draft' && (
                            <>
                              ...
                              {/* Limites pré-aprovados — visão bancária */}
                              {creditoData?.limites && (
                                <div style={{ margin: '0 20px 16px', padding: '16px', background: 'rgba(201,154,61,0.04)', border: '1px solid rgba(201,154,61,0.2)', borderRadius: '6px' }}>
                                  <div style={{ fontSize: '0.62rem', fontWeight: 800, color: 'var(--color-gold)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '10px' }}>
                                    <i className="fa-solid fa-circle-check" style={{ marginRight: '5px' }}></i>Crédito pré-aprovado para sua jornada
                                  </div>
                                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                                    <div style={{ textAlign: 'center' }}>
                                      <div style={{ fontSize: '0.58rem', color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '3px' }}>Cartão rotativo</div>
                                      <div style={{ fontSize: '1.1rem', fontWeight: 900, color: creditoData.limites.cartao?.disponivel ? 'var(--color-green)' : 'var(--text-muted)', fontFamily: 'monospace' }}>
                                        {creditoData.limites.cartao?.disponivel ? `R$ ${creditoData.limites.cartao.valor.toLocaleString('pt-BR')}` : '—'}
                                      </div>
                                    </div>
                                    <div style={{ textAlign: 'center' }}>
                                      <div style={{ fontSize: '0.58rem', color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '3px' }}>Empréstimo produtivo</div>
                                      <div style={{ fontSize: '1.1rem', fontWeight: 900, color: creditoData.limites.emprestimo?.disponivel ? 'var(--color-gold)' : 'var(--text-muted)', fontFamily: 'monospace' }}>
                                        {creditoData.limites.emprestimo?.disponivel ? `R$ ${creditoData.limites.emprestimo.valor.toLocaleString('pt-BR')}` : '—'}
                                      </div>
                                    </div>
                                  </div>
                                  <p style={{ fontSize: '0.62rem', color: 'var(--text-muted)', marginTop: '8px', lineHeight: '1.35' }}>
                                    Baseado na sua operação iFood dos últimos 90 dias. Preencha a solicitação abaixo para liberar o crédito.
                                  </p>
                                </div>
                              )}
                                ...
                                <div className="aval-peers-list">
                                  ...
                                  <div className="peer-item-card glass" style={{ flexDirection: 'column', alignItems: 'stretch', gap: '8px' }}>
                                    ...
                                    {avaliacoes.marcos === 'pending' ? (
                                      ...
                                      <p style={{ fontSize: '0.75rem', fontStyle: 'italic', color: 'var(--text-secondary)' }}>
                                        &ldquo;{avaliacoes.marcosComment}&rdquo;
                                      </p>
                                    )}
                                  </div>
                                  ...
                                  <div className="peer-item-card glass" style={{ flexDirection: 'column', alignItems: 'stretch', gap: '8px' }}>
                                    ...
                                    {avaliacoes.aline === 'pending' ? (
                                      ...
                                      <p style={{ fontSize: '0.75rem', fontStyle: 'italic', color: 'var(--text-secondary)' }}>
                                        &ldquo;{avaliacoes.alineComment}&rdquo;
                                      </p>
                                    )}
                                  </div>
                                </div>
                                  ...
                                  <div className="premium-credit-card compact-slip" style={{
                                    ...
                                    <div className="slip-footer" style={{ borderTop: '1px dashed var(--border-color)', paddingTop: '10px', marginTop: '10px', display: 'block', height: 'auto' }}>
                                      <p style={{ fontSize: '0.72rem', color: 'var(--text-secondary)', fontStyle: 'italic', lineHeight: '1.4', textAlign: 'center', margin: 0 }}>
                                        &ldquo;Este ciclo fortaleceu a jornada de {membro.nome.split(' ')[0]} e adicionou contribuição ao Fundo Abias.&rdquo;
                                      </p>
                                    </div>
                                  </div>
                                ) : (
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                <div style={{ background: 'rgba(255,255,255,0.02)', padding: '12px', border: '1px solid var(--border-color)' }}>
                                  ...
                                  <p style={{ fontSize: '0.72rem', fontStyle: 'italic', color: 'var(--text-secondary)', lineHeight: '1.4' }}>
                                    &ldquo;João é parceiro de confiança, conheço a rota dele na Zona Leste.&rdquo;
                                  </p>
                                </div>
                                <div style={{ background: 'rgba(255,255,255,0.02)', padding: '12px', border: '1px solid var(--border-color)' }}>
                                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px', fontSize: '0.75rem' }}>
                                  ...
                                  <p style={{ fontSize: '0.72rem', fontStyle: 'italic', color: 'var(--text-secondary)', lineHeight: '1.4' }}>
                                    &ldquo;Grande profissional de entrega. Corre garantido no asfalto.&rdquo;
                                  </p>
                                </div>
                              </div>
                            </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                      <div className="milestone-card glass">
                        ...
                        <div className="q-details" style={{ marginBottom: '10px' }}>
                          ...
                          <span>Descrição: &ldquo;{currentDescricao}&rdquo;</span>
                        </div>
                      ...
                      {cicloEstado === 'evidence_review' && (
                        <div className="glass" style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                          ...
                          <div style={{ background: 'rgba(255,255,255,0.03)', padding: '10px', borderRadius: '4px', fontSize: '0.75rem' }}>
                            ...
                            <p><strong>Observações João:</strong> &ldquo;{evidencia.obs || 'Nenhuma obs.'}&rdquo;</p>
                          </div>
              ...
              {profileMode === 'gestao' && (
                <div className="screen active" id="screen-admin">
                  ...
                  <div className="admin-container">
                    ...
                    {/* iFood Credit Analysis Panel */}
                    <div className="admin-queue-card glass">
                      <h4 style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        Análise Operacional iFood
                        <span style={{ fontSize: '0.65rem', color: 'var(--text-muted)', fontWeight: 400, letterSpacing: '0.05em' }}>CRÉDITO SUGERIDO</span>
                      </h4>
                      {gestaoLoading ? (
                        <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
                          <i className="fa-solid fa-circle-notch fa-spin" style={{ marginRight: '6px' }}></i>Carregando dados iFood...
                        </p>
                      ) : gestaoData && gestaoData.length > 0 ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                          {gestaoData.map(m => {
                            const rec = m.recomendacao
                            const recColor = rec === 'APROVAR' ? 'var(--color-green)' : rec === 'ANALISAR' ? 'var(--color-gold)' : rec === 'REVISAR' ? 'var(--color-terra)' : 'var(--text-muted)'
                            const recBg = rec === 'APROVAR' ? 'rgba(22,61,47,0.3)' : rec === 'ANALISAR' ? 'rgba(201,154,61,0.2)' : rec === 'REVISAR' ? 'rgba(154,79,47,0.2)' : 'rgba(255,255,255,0.03)'
                            return (
                              <div key={m.membroId} style={{ background: 'rgba(255,255,255,0.015)', padding: '12px', border: '1px solid var(--border-color)', borderRadius: '4px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                                  <div>
                                    <span style={{ fontWeight: 800, fontSize: '0.8rem' }}>{m.nome}</span>
                                    <span style={{ fontSize: '0.68rem', color: 'var(--text-secondary)', marginLeft: '8px' }}>{m.ferramenta}</span>
                                  </div>
                                  <span style={{ fontSize: '0.6rem', fontWeight: 800, padding: '3px 8px', background: recBg, color: recColor, border: `1px solid ${recColor}33`, borderRadius: '3px' }}>
                                    {rec}
                                  </span>
                                </div>
                                {m.ifood ? (
                                  <>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '5px', marginBottom: '6px' }}>
                                      {[
                                        { label: 'Score iFood', val: m.ifood.scoreIfood },
                                        { label: 'Ganho/sem', val: `R$ ${Number(m.ifood.ganhoMedioSemanal).toFixed(0)}` },
                                        { label: 'Entregas/90d', val: m.ifood.entregasRealizadas },
                                        { label: 'Avaliação', val: `${Number(m.ifood.avaliacaoMedia).toFixed(2)} ★` },
                                        { label: 'Cancelam.', val: `${(Number(m.ifood.taxaCancelamento) * 100).toFixed(1)}%` },
                                        { label: 'Dias ativos', val: m.ifood.diasAtivos }
                                      ].map(({ label, val }) => (
                                        <div key={label} style={{ background: 'rgba(255,255,255,0.02)', padding: '5px 4px', textAlign: 'center', borderRadius: '3px' }}>
                                          <div style={{ fontWeight: 800, fontSize: '0.7rem', color: '#ffffff' }}>{val}</div>
                                          <div style={{ color: 'var(--text-muted)', fontSize: '0.56rem', marginTop: '1px' }}>{label}</div>
                                        </div>
                                      ))}
                                    </div>

                                    {/* Linha 2: créditos isolados */}
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5px' }}>
                                      {/* Cartão */}
                                      <div style={{
                                        padding: '8px', borderRadius: '4px', textAlign: 'center',
                                        background: m.limites?.cartao?.disponivel ? 'rgba(22,61,47,0.2)' : 'rgba(255,255,255,0.02)',
                                        border: `1px solid ${m.limites?.cartao?.disponivel ? 'rgba(22,61,47,0.5)' : 'var(--border-color)'}`
                                      }}>
                                        <div style={{ fontSize: '0.56rem', color: 'var(--text-muted)', marginBottom: '2px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Cartão</div>
                                        <div style={{ fontWeight: 900, fontSize: '0.82rem', color: m.limites?.cartao?.disponivel ? 'var(--color-green)' : 'var(--text-muted)' }}>
                                          {m.limites?.cartao?.disponivel ? `R$ ${m.limites.cartao.valor}` : '—'}
                                        </div>
                                        <div style={{ fontSize: '0.52rem', color: 'var(--text-muted)', marginTop: '1px' }}>Limite rotativo</div>
                                      </div>

                                      {/* Empréstimo */}
                                      <div style={{
                                        padding: '8px', borderRadius: '4px', textAlign: 'center',
                                        background: m.limites?.emprestimo?.disponivel ? 'rgba(201,154,61,0.1)' : 'rgba(255,255,255,0.02)',
                                        border: `1px solid ${m.limites?.emprestimo?.disponivel ? 'rgba(201,154,61,0.3)' : 'var(--border-color)'}`
                                      }}>
                                        <div style={{ fontSize: '0.56rem', color: 'var(--text-muted)', marginBottom: '2px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Empréstimo</div>
                                        <div style={{ fontWeight: 900, fontSize: '0.82rem', color: m.limites?.emprestimo?.disponivel ? 'var(--color-gold)' : 'var(--text-muted)' }}>
                                          {m.limites?.emprestimo?.disponivel ? `R$ ${m.limites.emprestimo.valor}` : '—'}
                                        </div>
                                        <div style={{ fontSize: '0.52rem', color: 'var(--text-muted)', marginTop: '1px' }}>
                                          {m.limites?.emprestimo?.disponivel ? 'Produtivo' : (m.limites?.emprestimo?.motivo || 'Não disponível')}
                                        </div>
                                      </div>
                                    </div>
                                  </>
                                ) : (
                                  <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)', fontStyle: 'italic', margin: 0 }}>Sem histórico iFood vinculado</p>
                                )}
                                {m.cicloAtivo && (
                                  <div style={{ marginTop: '8px', padding: '5px 8px', background: 'rgba(201,154,61,0.05)', border: '1px solid rgba(201,154,61,0.15)', borderRadius: '3px', fontSize: '0.68rem' }}>
                                    <span style={{ color: 'var(--color-gold)', fontWeight: 700 }}>Ciclo ativo</span>
                                    {' '}R$ {Number(m.cicloAtivo.valor).toFixed(2)} — {m.cicloAtivo.finalidade}
                                    <span style={{ color: 'var(--text-muted)', marginLeft: '4px' }}>({m.cicloAtivo.estado})</span>
                                  </div>
                                )}
                                <div style={{ marginTop: '8px' }}>
                                  {parecerAi[m.membroId] ? (
                                    <div style={{ background: 'rgba(201,154,61,0.05)', border: '1px solid rgba(201,154,61,0.2)', borderRadius: '4px', padding: '10px' }}>
                                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                                        <span style={{ fontSize: '0.65rem', fontWeight: 800, color: 'var(--color-gold)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                                          <i className="fa-solid fa-robot" style={{ marginRight: '4px' }}></i>Parecer IA Abias
                                        </span>
                                        {(() => {
                                          const r = parecerAi[m.membroId].parecer?.recomendacao
                                          const color = r === 'APROVAR' ? 'var(--color-green)' : r === 'ANALISAR' ? 'var(--color-gold)' : r === 'REVISAR' ? 'var(--color-terra)' : 'var(--danger)'
                                          const bg = r === 'APROVAR' ? 'rgba(22,61,47,0.4)' : r === 'ANALISAR' ? 'rgba(201,154,61,0.2)' : r === 'REVISAR' ? 'rgba(154,79,47,0.2)' : 'rgba(239,68,68,0.15)'
                                          return <span style={{ fontSize: '0.58rem', fontWeight: 800, padding: '2px 7px', background: bg, color, borderRadius: '3px', border: `1px solid ${color}55` }}>{r}</span>
                                        })()}
                                      </div>
                                      <p style={{ fontSize: '0.72rem', color: 'var(--text-secondary)', lineHeight: '1.4', marginBottom: '8px' }}>
                                        {parecerAi[m.membroId].parecer?.analise}
                                      </p>
                                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5px', marginBottom: '8px' }}>
                                        <div style={{ padding: '7px', background: 'rgba(22,61,47,0.15)', border: '1px solid rgba(22,61,47,0.3)', borderRadius: '3px', textAlign: 'center' }}>
                                          <div style={{ fontSize: '0.55rem', color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '2px' }}>Cartão IA</div>
                                          <div style={{ fontWeight: 900, fontSize: '0.78rem', color: parecerAi[m.membroId].parecer?.limiteCartao?.disponivel ? 'var(--color-green)' : 'var(--text-muted)' }}>
                                            {parecerAi[m.membroId].parecer?.limiteCartao?.disponivel ? `R$ ${parecerAi[m.membroId].parecer.limiteCartao.valor}` : '—'}
                                          </div>
                                          <div style={{ fontSize: '0.5rem', color: 'var(--text-muted)', marginTop: '2px', lineHeight: '1.3' }}>{parecerAi[m.membroId].parecer?.limiteCartao?.justificativa}</div>
                                        </div>
                                        <div style={{ padding: '7px', background: 'rgba(201,154,61,0.08)', border: '1px solid rgba(201,154,61,0.2)', borderRadius: '3px', textAlign: 'center' }}>
                                          <div style={{ fontSize: '0.55rem', color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '2px' }}>Empréstimo IA</div>
                                          <div style={{ fontWeight: 900, fontSize: '0.78rem', color: parecerAi[m.membroId].parecer?.limiteEmprestimo?.disponivel ? 'var(--color-gold)' : 'var(--text-muted)' }}>
                                            {parecerAi[m.membroId].parecer?.limiteEmprestimo?.disponivel ? `R$ ${parecerAi[m.membroId].parecer.limiteEmprestimo.valor}` : '—'}
                                          </div>
                                          <div style={{ fontSize: '0.5rem', color: 'var(--text-muted)', marginTop: '2px', lineHeight: '1.3' }}>{parecerAi[m.membroId].parecer?.limiteEmprestimo?.justificativa}</div>
                                        </div>
                                      </div>
                                      {parecerAi[m.membroId].parecer?.fatoresPositivos?.length > 0 && (
                                        <div style={{ marginBottom: '6px' }}>
                                          <div style={{ fontSize: '0.58rem', color: 'var(--color-green)', fontWeight: 800, marginBottom: '3px' }}>+ PONTOS FORTES</div>
                                          {parecerAi[m.membroId].parecer.fatoresPositivos.map((f, i) => (
                                            <div key={i} style={{ fontSize: '0.65rem', color: 'var(--text-secondary)', marginBottom: '2px' }}>• {f}</div>
                                          ))}
                                        </div>
                                      )}
                                      {parecerAi[m.membroId].parecer?.pontosAtencao?.length > 0 && (
                                        <div style={{ marginBottom: '6px' }}>
                                          <div style={{ fontSize: '0.58rem', color: 'var(--color-terra)', fontWeight: 800, marginBottom: '3px' }}>! PONTOS DE ATENÇÃO</div>
                                          {parecerAi[m.membroId].parecer.pontosAtencao.map((p, i) => (
                                            <div key={i} style={{ fontSize: '0.65rem', color: 'var(--text-secondary)', marginBottom: '2px' }}>• {p}</div>
                                          ))}
                                        </div>
                                      )}
                                      {parecerAi[m.membroId].parecer?.consideracaoEquidade && (
                                        <div style={{ padding: '6px 8px', background: 'rgba(213,63,140,0.05)', border: '1px solid rgba(213,63,140,0.2)', borderRadius: '3px', marginBottom: '8px' }}>
                                          <div style={{ fontSize: '0.55rem', color: '#d53f8c', fontWeight: 800, marginBottom: '2px' }}>PERSPECTIVA DE EQUIDADE</div>
                                          <div style={{ fontSize: '0.65rem', color: 'var(--text-secondary)', lineHeight: '1.35' }}>{parecerAi[m.membroId].parecer.consideracaoEquidade}</div>
                                        </div>
                                      )}
                                      <button
                                        onClick={() => setParecerAi(prev => { const n = { ...prev }; delete n[m.membroId]; return n })}
                                        style={{ fontSize: '0.6rem', background: 'transparent', border: '1px solid var(--border-color)', color: 'var(--text-muted)', padding: '3px 8px', cursor: 'pointer', borderRadius: '2px' }}>
                                        Fechar parecer
                                      </button>
                                    </div>
                                  ) : (
                                    <button
                                      onClick={() => handleSolicitarParecerAi(m.membroId)}
                                      disabled={!!parecerAiLoading[m.membroId]}
                                      style={{ width: '100%', padding: '7px', fontSize: '0.7rem', background: 'rgba(201,154,61,0.07)', border: '1px solid rgba(201,154,61,0.25)', color: 'var(--color-gold)', cursor: parecerAiLoading[m.membroId] ? 'wait' : 'pointer', borderRadius: '3px', fontWeight: 700 }}>
                                      {parecerAiLoading[m.membroId]
                                        ? <><i className="fa-solid fa-circle-notch fa-spin" style={{ marginRight: '5px' }}></i>Analisando perfil...</>
                                        : <><i className="fa-solid fa-robot" style={{ marginRight: '5px' }}></i>Solicitar Parecer IA</>
                                      }
                                    </button>
                                  )}
                                </div>
                        ...
                        <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Nenhum membro cadastrado na plataforma.</p>
                      )}
                    </div>
                      ...
                      {!solicitacao || ['draft', 'submitted', 'community_validation', 'partner_quote'].includes(cicloEstado) ? (
                        ...
                        <div className="queue-item">
                          ...
                          <div className="q-details">
                            ...
                            {currentDescricao && <span>Descrição: &ldquo;{currentDescricao}&rdquo;</span>}
                          </div>
